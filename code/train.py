...
def main():
    ...
    for epoch in range(start_epoch, NUM_EPOCH + 1):
        print('\n===> Training Epoch: [%d/%d]...  Learning Rate: %.9f'%(epoch,
                                                                      NUM_EPOCH,
        ...
        writer.add_scalar('Train/lr', solver.get_current_learning_rate(), epoch)
        writer.add_scalar('Train/Loss', sum(train_loss_list)/len(train_set), epoch)
        ...
        ssim_list = []
        val_loss_list = []
        with tqdm(total=len(val_loader), desc='Epoch: [%d/%d]' % (epoch, NUM_EPOCH), miniters=1) as t:
        ...
        writer.add_scalar('Val/Loss', sum(val_loss_list)/len(val_loss_list), epoch)

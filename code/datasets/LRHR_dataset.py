...
if __name__ == '__main__':
    ...
    opt['phase'] = 'train'
    opt['data_type'] = 'img'
    opt['dataroot_HR'] = '/train_data/super-resolution/Search_F_Youku/train/HR'
    opt['dataroot_LR'] = '/train_data/super-resolution/Search_F_Youku/train/LR'
    opt['LR_size'] = 32
    opt['scale'] = 4 # not used in there
    opt['rgb_range'] = 255
    opt['noise'] = '.'
    yoku_dataset = LRHRDataset(opt)
    dataloader = data.DataLoader(yoku_dataset, batch_size=32, shuffle=True, num_workers=4, pin_memory=True)
    for idx, data_map in enumerate(dataloader):
        print('data: ', data_map['LR'].size())
